...
myWorkspaces = ["main", "dev", "www", "doc", "http", "work", "mus", "git", "nix"]
...
--        , ("M-<F2>", spawn "/bin/ls ~/wallpapers | shuf -n 1 | xargs xwallpaper --stretch")
    ...
    -- Xbacklight keys
       ...
       -- multiply with shift
        ...
        where
            showHelp = spawn $ unwords [ "/etc/nixos/modules/xmonad/nightingale/xmobar/showHintForKeymap.sh"
                                , desc
                                , "dzen_xmonad"
                                , show (30 :: Integer)
                                , show (0 :: Integer)
...
keyMapDoc desc id color delay = do
  -- focused screen location/size
    handle <- spawnPipe $ unwords [ "/etc/nixos/modules/xmonad/nightingale/xmobar/showHintForKeymap.sh"
                                  , desc
                                  , id
                                  , "22"
                                  , show delay
...
main = do
    ...
    xmproc0 <- spawnPipe "xmobar -x 0 /etc/nixos/modules/xmonad/nightingale/xmobar/xmobarrc0"
